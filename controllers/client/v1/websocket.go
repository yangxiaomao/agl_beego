 ...
 * @LastEditTime: 2020-12-16 09:32:30
...
func (this *WebSocketController) DiagramData() {
	...
	for {
		...
		for {
			...
			categoriesData = append(categoriesData, "09:"+strconv.Itoa(rand.Intn(20)))

			if len(categoriesData) >= indexNum {
				cateindex := len(categoriesData) - indexNum
				categoriesData = categoriesData[cateindex:]
			}

			seriesData = append(seriesData, rand.Intn(20))
			if len(seriesData) >= indexNum {
				seriesindex := len(seriesData) - indexNum
				seriesData = seriesData[seriesindex:]
			}
